...
const localPath = (u) => {
  const url = new URL(u);
  return url.host === 'meaningisthemission.com'
    ? path.join(PUB, url.pathname)
    : path.join(PUB, 'wp/ext', url.host, url.pathname);
};

/** Files the markup never links but the site still serves. None on this site. */
const EXTRA = [];
...
const workers = Array.from({ length: 8 }, async () => {
  while (queue.length) {
    ...
    try {
      const res = await fetch(u, { headers: { 'user-agent': UA, referer: 'https://meaningisthemission.com/' } });
